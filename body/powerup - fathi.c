...
void spawnPowerUps()
{
    ...
    for (int i = 0; i < MAX_POWERUPS; i++)
    {
        ...
        powerUps[i].y = row * TILE_SIZE + TILE_SIZE / 2; // Pastikan TILE_SIZE terdefinisi
        powerUps[i].type = (rand() % 3) + 1;
        powerUps[i].active = 1;

        // Notifikasi ke terminal
        printf("Power-up %d (Tipe %d) dimunculkan:\n", i + 1, powerUps[i].type);
        printf("  -> Koordinat Grid   : Baris %d, Kolom %d\n", row, col);
        printf("  -> Koordinat Piksel : X = %d, Y = %d\n", powerUps[i].x, powerUps[i].y);
    }
    printf("-----------------------------------\n");
}
